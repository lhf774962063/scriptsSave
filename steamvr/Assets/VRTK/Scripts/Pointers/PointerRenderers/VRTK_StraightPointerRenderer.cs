...
namespace VRTK
{
    ...
    public class VRTK_StraightPointerRenderer : VRTK_BasePointerRenderer
    {
        ...
        protected virtual float CastRayForward()
        {
            ...
            //下面这句修改了~；把~layersToIgnore 修改成了layersToIgnore；
            var rayHit = Physics.Raycast(pointerRaycast, out pointerCollidedWith, maximumLength, layersToIgnore);

            //     var bHit = Physics.Raycast(pointerRaycast, out pointerCollidedWith, maximumLength, layersToIgnore);
            ...
            if (rayHit)
            {
                ...
                {
                    //在这里可以做很多事情；可以检测被射线射中的物体的名字、tag、层等等；然后把这些信息传递出去使用；
                    if (gameObject.name == "LeftController")
                    {
                      //  Debug.Log(gameObject.name);
                        //  target.transform.position = pointerCollidedWith.normal.x

                        leftTarget.transform.position = new Vector3(pointerCollidedWith.point.x, pointerCollidedWith.point.y, pointerCollidedWith.point.z);
                        if (pointerCollidedWith.rigidbody)
                        {
                            
                        }
                    }
                    else if (gameObject.name == "RightController")
                    {
                      //  Debug.Log(gameObject.name);
                           rightTarget.transform.position = new Vector3(pointerCollidedWith.point.x, pointerCollidedWith.point.y, pointerCollidedWith.point.z);
            ...
            else
            {
                ...
                if (gameObject.name == "LeftController")
                {
                    leftTarget.transform.position = new Vector3(10000, -10000, 1000);


                }
                if (gameObject.name == "RightController")
                {
                    rightTarget.transform.position = new Vector3(10000, -10000, 1000);
        ...
        //下面这个函数是vrtk原来的函数；因为需要被我改成了上面这个函数； 
        //protected virtual float CastRayForward()
        //{
        //    Transform origin = GetOrigin();
        //    Ray pointerRaycast = new Ray(origin.position, origin.forward);
